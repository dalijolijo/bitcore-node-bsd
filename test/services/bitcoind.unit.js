...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/bitsend.conf')));
var BitcoinService = proxyquire('../../lib/services/bitcoind', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultBitcoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.bitsend.conf'), 'utf8');
...
describe('Bitcoin Service', function() {
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a bitsend.conf file', function() {
      ...
      bitcoind.options.spawn.datadir = '/tmp/.bitsend';
      var node = {};
    ...
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var TestBitcoin = proxyquire('../../lib/services/bitcoind', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badbitsend.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the bitsend.conf file', function() {
      var bitcoind = new BitcoinService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default rpc port for testnet', function() {
      ...
      var bitcoind = new BitcoinService(config);
      bitcoind._getNetworkConfigPath().should.equal('testnet3/bitsend.conf');
    });
    it('will get default rpc port for regtest', function() {
      bitcore.Networks.enableRegtest();
      var config = {
      ...
      var bitcoind = new BitcoinService(config);
      bitcoind._getNetworkConfigPath().should.equal('regtest/bitsend.conf');
    });
  });
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      bitcoind.spawn.configPath = 'testdir/bitsend.conf';
    ...
    it('will respawn bitcoind spawned process', function(done) {
      ...
      bitcoind.spawn.configPath = '/tmp/bitcoin/bitsend.conf';
      bitcoind.spawn.config = {};
