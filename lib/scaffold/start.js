...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for bitsendd is necessary for this upgrade with the "reindex=1" bitsend.conf option. \n' +
                 'There are changes necessary in both bitsend.conf and bitcore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/bitsendd')
            }
          }
        }
      };
